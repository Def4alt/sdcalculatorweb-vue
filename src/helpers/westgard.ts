export default class Westgard {
	...
	Rule22S(index: number): boolean {
		return (
			(this.isValueExceedsPlus2SD(this.averageValues[index]) &&
	...
	check(
		...
		for (let i = 1; i < this.averageValues.length; i++) {
			...
			if (
				numbersLeft >= 4 &&
				this.Rule41S(i) &&
				previousWarnings[i - 1] !== "41S"
			)
				return "41S";

			if (numbersLeft >= 2) {
				if (this.Rule22S(i) && previousWarnings[i - 1] !== "22S")
					return "22S";
				else if (this.RuleR4S(i) && previousWarnings[i - 1] !== "R4S")
					return "R4S";
			}
